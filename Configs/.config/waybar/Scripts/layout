...
if [[ -f "$ACTIVE_FILE" ]]; then
    ...
    case "$active_profile" in
        study)  exec "$HOME/.config/rofi/scripts/study.sh" ;;
        coding) exec "$HOME/.config/rofi/scripts/coding.sh" ;;
    esac
fi
...
chosen=$(echo -e "$entries" | rofi -dmenu -i -p " Layout" \
    -theme "$HOME/.config/rofi/themes/presets/submenu.rasi") || true
[[ -z "$chosen" ]] && exit 0
