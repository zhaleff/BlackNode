...
hl.exec_cmd("sleep 1 && ~/.local/bin/package.sh")
    ...
    -- psychological notification daemons (loop forever in background)
    hl.exec_cmd("sleep 10 && (while true; do ~/.local/bin/persevere.sh; sleep 300; done &)")
    hl.exec_cmd("sleep 10 && (while true; do sleep 7200; ~/.local/bin/identity-reinforce.sh; done &)")
    hl.exec_cmd("sleep 10 && (~/.local/bin/water-reminder.sh &)")
     hl.exec_cmd("sleep 30 && ~/.local/bin/blacknode brain start")
end)
