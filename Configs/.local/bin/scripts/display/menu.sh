...
# ============================================
# BLACKNODE DISPLAY - simple with notifications
# ============================================
...
main() {
    ...
    local choice
    choice=$(printf '%s\n' "${options[@]}" | rofi -dmenu -i -p "Display" -theme "$ROFI_THEME")
...
change_res() {
    local res
    res=$(printf '%s\n' "1366x768" "1920x1080" | rofi -dmenu -i -p "Resolution" -theme "$ROFI_THEME")
    if [[ -n "$res" ]]; then
...
change_rate() {
    local rate
    rate=$(printf '%s\n' "60" "48" "144" | rofi -dmenu -i -p "Rate" -theme "$ROFI_THEME")
    if [[ -n "$rate" ]]; then
